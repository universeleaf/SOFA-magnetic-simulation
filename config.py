...
ELASTICROD_REALTIME_NODE_COUNT = 101
# Strict GUI now uses a separately revalidated 81-node profile. The earlier 91-node
# + oversized GUI dt combination looked fast in the old headless harness because the
# harness accidentally kept a fixed dt; with the real dynamic dt path it blew up near
# the entry. Headless diagnostics keep the 101-node model, while runSofa GUI uses a
# lighter but actually stable profile.
ELASTICROD_GUI_NODE_COUNT = 101
ELASTICROD_REALTIME_STARTUP_RAMP_S = 0.15
ELASTICROD_REALTIME_DT_FREE_S = 3.0e-3 if ELASTICROD_STABILIZATION_MODE == 'strict' else 2.0e-3
ELASTICROD_REALTIME_DT_TRANSITION_S = 2.5e-3 if ELASTICROD_STABILIZATION_MODE == 'strict' else 1.0e-3
ELASTICROD_REALTIME_DT_CONTACT_S = 1.0e-3 if ELASTICROD_STABILIZATION_MODE == 'strict' else 5.0e-4
...
ELASTICROD_GUI_WALLCLOCK_INSERTION_DT_MAX_S = 0.005
ELASTICROD_GUI_WALLCLOCK_PUSH_SPEED_SCALE = 1.0
ELASTICROD_GUI_WALLCLOCK_STARTUP_RAMP_S = 0.45
ELASTICROD_GUI_WALLCLOCK_INSERTION_DT_FREE_SCALE = 1.75
ELASTICROD_GUI_WALLCLOCK_INSERTION_DT_TRANSITION_SCALE = 1.20
ELASTICROD_GUI_WALLCLOCK_INSERTION_DT_CONTACT_SCALE = 1.35
...
ELASTICROD_GUI_DT_FREE_S = 3.0e-3 if ELASTICROD_STABILIZATION_MODE == 'strict' else 2.0e-3
ELASTICROD_GUI_DT_TRANSITION_S = 2.5e-3 if ELASTICROD_STABILIZATION_MODE == 'strict' else 1.0e-3
ELASTICROD_GUI_DT_CONTACT_S = 1.0e-3 if ELASTICROD_STABILIZATION_MODE == 'strict' else 5.0e-4
ELASTICROD_GUI_SOLVER_MAX_ITER_FREE = 64 if ELASTICROD_STABILIZATION_MODE == 'strict' else 80
ELASTICROD_GUI_SOLVER_MAX_ITER_TRANSITION = 64 if ELASTICROD_STABILIZATION_MODE == 'strict' else 140
ELASTICROD_GUI_SOLVER_MAX_ITER_CONTACT = 320 if ELASTICROD_STABILIZATION_MODE == 'strict' else 220
ELASTICROD_GUI_SOLVER_TOL_FREE = 1.5e-4 if ELASTICROD_STABILIZATION_MODE == 'strict' else 1.0e-4
ELASTICROD_GUI_SOLVER_TOL_TRANSITION = 1.0e-4 if ELASTICROD_STABILIZATION_MODE == 'strict' else 5.0e-5
ELASTICROD_GUI_SOLVER_TOL_CONTACT = 5.0e-6 if ELASTICROD_STABILIZATION_MODE == 'strict' else 1.0e-5
ELASTICROD_REALTIME_SOLVER_MAX_ITER_FREE = 64 if ELASTICROD_STABILIZATION_MODE == 'strict' else 150
ELASTICROD_REALTIME_SOLVER_MAX_ITER_TRANSITION = 64 if ELASTICROD_STABILIZATION_MODE == 'strict' else 250
ELASTICROD_REALTIME_SOLVER_MAX_ITER_CONTACT = 320 if ELASTICROD_STABILIZATION_MODE == 'strict' else 400
ELASTICROD_REALTIME_SOLVER_TOL_FREE = 1.5e-4 if ELASTICROD_STABILIZATION_MODE == 'strict' else 5.0e-5
ELASTICROD_REALTIME_SOLVER_TOL_TRANSITION = 1.0e-4 if ELASTICROD_STABILIZATION_MODE == 'strict' else 1.0e-5
ELASTICROD_REALTIME_SOLVER_TOL_CONTACT = 5.0e-6 if ELASTICROD_STABILIZATION_MODE == 'strict' else 5.0e-6
ELASTICROD_REALTIME_SPEED_SCALE_FREE = 2.8
ELASTICROD_REALTIME_SPEED_SCALE_TRANSITION = 1.9
ELASTICROD_REALTIME_SPEED_SCALE_CONTACT = 1.0
...
# Strict defaults to a physics-only path: no post-solve position correction or
# lumen clamping. Real rod elasticity, real SOFA contact, and magnetic torque
# remain active.
ELASTICROD_ENABLE_STRICT_POSTSOLVE_GUARD = ELASTICROD_STABILIZATION_MODE != 'strict'
ELASTICROD_ENABLE_STRICT_LUMEN_CLAMP = ELASTICROD_STABILIZATION_MODE != 'strict'
ELASTICROD_STRICT_LUMEN_CLAMP_TOLERANCE_MM = 0.02
ELASTICROD_STRICT_MAX_LINEAR_SPEED_MM_S = 2000.0
ELASTICROD_STRICT_NATIVE_LUMEN_BARRIER = True
ELASTICROD_STRICT_EXTERNAL_SUPPORT_LENGTH_MM = 6.0
ELASTICROD_STRICT_EXTERNAL_PUSH_FORCE_N = 0.45
ELASTICROD_STRICT_EXTERNAL_PUSH_FORCE_MAX_N = 0.60
...
ELASTICROD_STRICT_SUPPORT_RELEASE_MM = 10.0
...
ELASTICROD_STRICT_BARRIER_ACTIVATION_MARGIN_MM = 0.65
ELASTICROD_STRICT_BARRIER_SAFETY_MARGIN_MM = 0.18
ELASTICROD_STRICT_BARRIER_STIFFNESS_N_PER_M = 8000.0
ELASTICROD_STRICT_BARRIER_DAMPING_N_S_PER_M = 0.90
ELASTICROD_STRICT_BARRIER_MAX_FORCE_PER_NODE_N = 2.20
ELASTICROD_STRICT_HEAD_STRETCH_SOFT_LIMIT = 0.01
ELASTICROD_STRICT_HEAD_STRETCH_LIMIT = 0.02
ELASTICROD_STRICT_GLOBAL_STRETCH_SOFT_LIMIT = 0.02
ELASTICROD_STRICT_GLOBAL_STRETCH_HARD_LIMIT = 0.03
ELASTICROD_STRICT_AXIAL_STIFFNESS_SCALE = 50.0
...
ELASTICROD_STRICT_INITIAL_STRAIGHT_PUSH_MM = 2.0
ELASTICROD_STRICT_MAGNETIC_RELEASE_SPAN_MM = 4.0
ELASTICROD_STRICT_MAGNETIC_LOOKAHEAD_MM = 18.0
ELASTICROD_STRICT_MAGNETIC_RECOVERY_LOOKAHEAD_MM = 8.0
ELASTICROD_STRICT_MAGNETIC_RECOVERY_SCALE_FLOOR = 0.85
ELASTICROD_STRICT_MAGNETIC_RECENTER = True
ELASTICROD_STRICT_MAGNETIC_ASSIST = False
...
ELASTICROD_STRICT_TIP_TARGET_FORCE_N = 0.0
ELASTICROD_STRICT_INITIAL_AXIS_HOLD_MM = 0.0
ELASTICROD_STRICT_BEND_LOOKAHEAD_MM = 18.0
ELASTICROD_STRICT_BEND_NEAR_WINDOW_MM = 5.0
ELASTICROD_STRICT_BEND_TURN_MEDIUM_DEG = 9.0
ELASTICROD_STRICT_BEND_TURN_HIGH_DEG = 20.0
ELASTICROD_STRICT_FIELD_SCALE_STRAIGHT = 0.55 if ELASTICROD_STRICT_MAGNETIC_PREVIEW_SCALING else 1.0
ELASTICROD_STRICT_FIELD_SCALE_BEND = 1.15 if ELASTICROD_STRICT_MAGNETIC_PREVIEW_SCALING else 1.0
ELASTICROD_STRICT_PUSH_SCALE_STRAIGHT_CONTACT = 0.98
ELASTICROD_STRICT_PUSH_SCALE_BEND_CONTACT = 0.86
ELASTICROD_STRICT_LIGHT_CONTACT_PUSH_SCALE = 0.94
ELASTICROD_STRICT_GUIDED_CONTACT_PUSH_SCALE = 0.90
...
ELASTICROD_STRICT_RECENTER_CLEARANCE_MM = 0.45 if ELASTICROD_STRICT_MAGNETIC_RECENTER else 0.0
ELASTICROD_STRICT_RECENTER_OFFSET_MM = 0.60 if ELASTICROD_STRICT_MAGNETIC_RECENTER else 0.0
ELASTICROD_STRICT_RECENTER_BLEND = 0.70 if ELASTICROD_STRICT_MAGNETIC_RECENTER else 0.0
ELASTICROD_STRICT_HEAD_STRETCH_RELIEF_START = 0.03
ELASTICROD_STRICT_HEAD_STRETCH_RELIEF_FULL = 0.10
...
ELASTICROD_STRICT_HEAD_STRETCH_FIELD_SCALE_FLOOR = 0.60
ELASTICROD_STRICT_GUI_EXACT_SURFACE_RECHECK_STEPS = 18
ELASTICROD_STRICT_GUI_SURFACE_REFRESH_NEAR_STEPS = 4
ELASTICROD_STRICT_GUI_SURFACE_REFRESH_FAR_STEPS = 18
...
ELASTICROD_STRICT_SIMPLE_TAIL_DRIVE = False
...
ELASTICROD_ENABLE_VIRTUAL_SHEATH = False
...
ELASTICROD_STRICT_HAND_PUSH_MIN_FORCE_SCALE = 1.0
ELASTICROD_STRICT_GUI_MAX_INSERTION_STEP_MM = 0.06
...
ELASTICROD_SHEATH_STIFFNESS_N_PER_M = 200.0
...
ELASTICROD_ENABLE_THRUST_LIMIT = ELASTICROD_STABILIZATION_MODE != 'strict'
ELASTICROD_THRUST_FORCE_N = 0.25
ELASTICROD_ENABLE_AXIAL_PATH_ASSIST = ELASTICROD_STABILIZATION_MODE != 'strict'
ELASTICROD_AXIAL_PATH_ASSIST_FORCE_N = 0.15
ELASTICROD_AXIAL_PATH_ASSIST_DEFICIT_MM = 0.05
...
ELASTICROD_BODY_YOUNG_PA = 55.0e9
ELASTICROD_BODY_POISSON = 0.33
ELASTICROD_BODY_DENSITY_KG_M3 = 6500.0
ELASTICROD_HEAD_EFFECTIVE_YOUNG_PA = 8.00e8
ELASTICROD_DISTAL_SOFT_LENGTH_MM = 20.0
# The NdFeB+PDMS composite head is much more torsion-compliant than an
# isotropic metal tip with the same effective bending modulus. Keep a separate
# shear scale so the distal segment can actually curl into bends instead of
# only forming a small shaft-driven yaw.
ELASTICROD_HEAD_EFFECTIVE_SHEAR_SCALE = 0.25
...
ELASTICROD_STRICT_SUPPORT_RELEASE_MM = max(float(NATIVE_WIRE_SPACING_MM), 1.0e-6)
...
ELASTICROD_CONTACT_FRICTION_MU = 0.01
...
ELASTICROD_GUIDEWIRE_CONTACT_STIFFNESS = 650.0 if ELASTICROD_STABILIZATION_MODE == 'strict' else 80.0
...
PUSH_FORCE_TARGET_SPEED_MM_S = 10.0
...
ELASTICROD_PROXIMAL_LATERAL_STIFFNESS_N_PER_M = (
    ...
    if (ELASTICROD_STABILIZATION_MODE == 'strict' and ELASTICROD_STRICT_DISABLE_NATIVE_BOUNDARY_DRIVER)
        ...
        if (ELASTICROD_STABILIZATION_MODE == 'strict' and ELASTICROD_PUSH_MODEL == 'introducer_feed')
            ...
            80.0
            if ELASTICROD_USE_KINEMATIC_SHEATH_DRIVER
            else (40.0 if ELASTICROD_ENABLE_VIRTUAL_SHEATH else 0.0)
        )
    )
)
...
ELASTICROD_PROXIMAL_LINEAR_DAMPING_N_S_PER_M = (
    ...
    if (ELASTICROD_STABILIZATION_MODE == 'strict' and ELASTICROD_STRICT_DISABLE_NATIVE_BOUNDARY_DRIVER)
        ...
        0.15
        if ELASTICROD_USE_KINEMATIC_SHEATH_DRIVER
        else (0.0 if ELASTICROD_STABILIZATION_MODE == 'strict' else 0.15)
    )
)
...
# strict 默认把这项关掉。
ELASTICROD_PROXIMAL_ANGULAR_DAMPING_NM_S_PER_RAD = 0.0
...
ELASTICROD_DISTRIBUTED_TRANSLATIONAL_DAMPING_N_S_PER_M = 0.09 if ELASTICROD_STABILIZATION_MODE == 'strict' else 0.02
ELASTICROD_DISTRIBUTED_TWIST_DAMPING_NM_S_PER_RAD = 0.006 if ELASTICROD_STABILIZATION_MODE == 'strict' else 0.005
...
ELASTICROD_MAGNETIC_HEAD_LENGTH_MM = min(
    float(ELASTICROD_DISTAL_SOFT_LENGTH_MM),
    24.0,
)
...
ELASTICROD_MAGNETIC_STRENGTH_SCALE = 3.00
...
ELASTICROD_STRICT_TIP_WALL_CONTACT_RELEASE_HOLD_STEPS = 4
...
MAGNETIC_LOOKAHEAD_DISTANCE_MM = 6.0
...
ELASTICROD_MAGNETIC_FIELD_SMOOTHING_ALPHA = 0.12 if ELASTICROD_STABILIZATION_MODE == 'strict' else 0.55
ELASTICROD_MAGNETIC_MAX_TURN_ANGLE_DEG = 50.0 if ELASTICROD_STABILIZATION_MODE == 'strict' else 75.0
ELASTICROD_MAGNETIC_FIELD_RAMP_TIME_S = 0.20 if ELASTICROD_STABILIZATION_MODE == 'strict' else 0.12
ELASTICROD_MAGNETIC_LATERAL_FORCE_SCALE = 0.0 if ELASTICROD_STABILIZATION_MODE == 'strict' else 0.15
...
ELASTICROD_RAYLEIGH_STIFFNESS = 1.0e-3 if ELASTICROD_STABILIZATION_MODE == 'safe' else 2.0e-5
ELASTICROD_RAYLEIGH_MASS = 0.10 if ELASTICROD_STABILIZATION_MODE == 'safe' else 0.05
...
def option_parameter_lines(backend_name: str | None = None) -> list[str]:
    ...
    return [
        ...
        f'  strictMagneticFlags(recenter/assist/previewScaling): {ELASTICROD_STRICT_MAGNETIC_RECENTER} / {ELASTICROD_STRICT_MAGNETIC_ASSIST} / {ELASTICROD_STRICT_MAGNETIC_PREVIEW_SCALING}',
