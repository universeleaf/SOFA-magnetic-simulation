...
ELASTICROD_REALTIME_NODE_COUNT = 121
# Double the previous strict GUI segment density so the guidewire no longer
# looks visibly piecewise in the first bends while keeping one tip node.
ELASTICROD_GUI_NODE_COUNT = 121
...
ELASTICROD_GUI_DT_CONTACT_S = 3.0e-3
...
ELASTICROD_GUI_SOLVER_MAX_ITER_CONTACT = 96 if ELASTICROD_STABILIZATION_MODE == 'strict' else 160
...
ELASTICROD_GUI_SOLVER_TOL_CONTACT = 2.6e-5 if ELASTICROD_STABILIZATION_MODE == 'strict' else 1.5e-5
...
ELASTICROD_REALTIME_CLEARANCE_TRANSITION_MM = 0.55
...
ELASTICROD_STRICT_GUI_EXACT_SURFACE_RECHECK_STEPS = 20
ELASTICROD_STRICT_GUI_SURFACE_REFRESH_NEAR_STEPS = 8
ELASTICROD_STRICT_GUI_SURFACE_REFRESH_FAR_STEPS = 36
ELASTICROD_STRICT_GUI_FAR_CLEARANCE_MM = 1.0
ELASTICROD_STRICT_GUI_FAR_OFFSET_MM = 1.0
ELASTICROD_VESSEL_QUERY_FACE_CANDIDATE_COUNT = 256
...
ELASTICROD_COLLISION_DECIMATION_STRIDE = (
    ...
    if (
        ELASTICROD_RUNTIME_GUI_NODE_BUDGET
        and ELASTICROD_GUI_NODE_COUNT >= 100
    )
    else 1
)
