...
elasticRod::elasticRod(MatrixXd initialNodes, MatrixXd undeformed, 
	...
	double m_youngM, double m_noMagYoungM, double m_shearM, double m_noMagShearM, double m_rodLength, double m_tubeRadius)
{
	...
	dt = m_dt;
	youngM = m_youngM;
	noMagYoungM = m_noMagYoungM;
	shearM = m_shearM;
	noMagShearM = m_noMagShearM;
	rho = m_rho;
	rodRadius = m_rodRadius;
	tubeRadius = m_tubeRadius;
	...
	tubeNv = 100;
	tubeNode.setZero(tubeNv+288, 3);
	tubeRadii.setZero(tubeNv+288);
	int temp = 0;
	...
	for (int i = 0; i < tubeNv; i++)
	{
		tubeRadii(tmp) = rodRadius*1.1;
		// tubeRadii(tmp) = 0.5;
		tmp = tmp + 1;
	}
	ifstream inFile2;
	inFile2.open("input/radius2.txt");
	double d;
	while(inFile2 >> d)
	{
		tubeRadii(tmp) = d - 1.0*rodRadius;
		// tubeRadii(tmp) = 0.5;
		tmp = tmp + 1;
	}
...
{
	for (int i = 0; i < ne - 5; i++)
	{
		EI(i) = (noMagYoungM*M_PI*rodRadius*rodRadius*rodRadius*rodRadius)/4;
		EA(i) =  noMagYoungM*M_PI*rodRadius*rodRadius;
		GJ(i) = (noMagShearM*M_PI*rodRadius*rodRadius*rodRadius*rodRadius)/2;
	}
	for (int i = ne - 5; i < ne; i++)
	{
		EI(i) = (youngM*M_PI*rodRadius*rodRadius*rodRadius*rodRadius)/4;
		EA(i) =  youngM*M_PI*rodRadius*rodRadius;
		GJ(i) = (shearM*M_PI*rodRadius*rodRadius*rodRadius*rodRadius)/2;
	}

	// double b = 1 * 10 * crossSectionalArea / EI / 0.1;
...
void elasticRod::updateNewtonX(VectorXd &DX, double a)
{
	for (int c=0; c < uncons; c++)
	{
...
void elasticRod::updateGuess(double a)
{
	for (int c=0; c < uncons; c++)
	{
		x[unconstrainedMap[c]] +=  a *  u[unconstrainedMap[c]] * dt;
	}
}
...
void elasticRod::computeMoment()
{
	...
	for (int i = 1; i < ne; i++)
	{
		moment(i, 0) = EI(i) / voronoiLen(i) * ( kappa(i, 0) - kappaBar(i, 0) );
		moment(i, 1) = EI(i) / voronoiLen(i) * ( kappa(i, 1) - kappaBar(i, 1) );
		moment(i, 2) = GJ(i) / voronoiLen(i) * ( deltam(i) + refTwist(i) - undeformedTwist(i) );
	}
}